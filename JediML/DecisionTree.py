...
class DecisionTree:
    ...
    def __init__(self, col=-1, value=None, true_branch=None, false_branch=None, each_class_counts=None, tree_height=0, is_leaf=True):
        self.col = col
        self.value = value
        self.true_branch = true_branch
        self.false_branch = false_branch
        self.each_class_counts = each_class_counts
        self.height_position = tree_height
...
class CART(ml.Learner):
    ...
    def load_train_data(self, input_data_file=''):
        ...
        self.train_X, self.train_Y = utility.LoadData.load(input_data_file, data_type='string')
    ...
    def load_test_data(self, input_data_file=''):
        ...
        self.test_X, self.test_Y = utility.LoadData.load(input_data_file, data_type='string')
    ...
    def init_W(self, mode='normal'):
        ...
        self.data_dimension = len(self.train_X[0])
        self.W = np.zeros(self.data_dimension)
    ...
    def grow_decision_tree_from(self, X, Y, height_position):
        ...
        for col in range(1, self.data_dimension):
        ...
        if best_gain > 0:
            ...
            if true_branch is None and false_branch is None:
                is_leaf = True
            if self.learn_type == 'classifier':
                return DecisionTree(col=best_attribute[0], value=best_attribute[1], true_branch=true_branch, false_branch=false_branch, each_class_counts=self.each_class_counts(Y), tree_height=height_position, is_leaf=is_leaf)
            elif self.learn_type == 'regression':
                data = [float(y) for y in Y]
                mean = sum(data) / len(data)
                mean_dict = {mean: 1}
                return DecisionTree(col=best_attribute[0], value=best_attribute[1], true_branch=true_branch, false_branch=false_branch, each_class_counts=mean_dict, tree_height=height_position, is_leaf=is_leaf)
        else:
            if self.learn_type == 'classifier':
                return DecisionTree(each_class_counts=self.each_class_counts(Y), tree_height=height_position, is_leaf=True)
            elif self.learn_type == 'regression':
                data = [float(y) for y in Y]
                mean = sum(data) / len(data)
                mean_dict = {mean: 1}
                return DecisionTree(each_class_counts=mean_dict, tree_height=height_position, is_leaf=True)
