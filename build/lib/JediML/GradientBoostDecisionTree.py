...
import JediML.DecisionTree as decision_tree
import JediML.LinearRegression as linear_regression
...
class Regression(ml.Learner):
    ...
    def load_train_data(self, input_data_file=''):
        ...
        self.train_X, self.train_Y = utility.LoadData.load(input_data_file)
    ...
    def load_test_data(self, input_data_file=''):
        ...
        self.test_X, self.test_Y = utility.LoadData.load(input_data_file)

        if (self.feature_transform_mode == 'polynomial') or (self.feature_transform_mode == 'legendre'):
            self.test_X = self.test_X[:, 1:]

            self.test_X = utility.LoadData.feature_transform(
                self.test_X,
                self.feature_transform_mode,
    ...
    def score_function(self, x, W):
        ...
        for i, weak_learner in enumerate(self.decision_tree):
            predicted_str = np.array(list(map(str, x)))
            predicted_str = ' '.join(predicted_str[1:])
            prediction = weak_learner.prediction(predicted_str, 'future_data')
    ...
    def calculate_alpha_s(self, weak_learner, s):
        ...
        for i in range(data_num):
            ...
            predicted_str = ' '.join(predicted_str[1:]) + ' ' + str(weak_learner.train_Y[i])
